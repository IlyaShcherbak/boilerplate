...
class Currency {
    ...
    constructor(value: number|string|Currency, opts?: Partial<options>) {
        ...
        let precision: number = pow(settings.precision);
        let v: number  = this.parse(value, settings);
    ...
    parse(value: number|string|Currency, opts: options, useRounding = true) {
        let v = 0;
        let { decimal, errorOnInvalid, precision: decimals } = opts; // opts ~= _settings
        let precision = pow(decimals);
        ...
        if (value instanceof Currency) {
            ...
            let decimalString = new RegExp('\\' + decimal, 'g');
            const formattedValue = value
                .replace(/\((.*)\)/, '-$1')   // allow negative e.g. (1.99)
                .replace(regex, '')           // replace any non numeric values
                .replace(decimalString, '.');  // convert any decimal values
            v = Number(formattedValue) * precision;   // scale number to integer value
            v = v || 0;
        } else {
            if (errorOnInvalid) {
    ...
    format(useSymbol: boolean): string {
        ...
        let values = String(this).replace(/^-/, '')
            .split('.');
